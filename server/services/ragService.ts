...
class RAGService {
  ...
  async findRelevantChunks(
    ...
    documentIds?: string[],
    topK = 5,
  ): Promise<{ chunk: DocumentChunk; similarity: number; documentTitle: string }[]> {
    try {
      // Generate embedding for the question
      const questionEmbedding = await embeddingService.generateEmbedding(question)

      // Build query
      const query: any = { ingestionStatus: "completed" }
      if (documentIds && documentIds.length > 0) {
        query._id = { $in: documentIds }
      }

      // Find documents with chunks
      const documents = await Document.find(query).select("title chunks")

      const relevantChunks: { chunk: DocumentChunk; similarity: number; documentTitle: string }[] = []
        ...
        if (doc.chunks && doc.chunks.length > 0) {
          for (const chunk of doc.chunks) {
            if (chunk.embeddings && chunk.embeddings.length > 0) {
              const similarity = embeddingService.cosineSimilarity(questionEmbedding, chunk.embeddings)
  ...
  async generateAnswer(question: string, relevantChunks: any[]): Promise<{ answer: string; confidence: number }> {
    try {
